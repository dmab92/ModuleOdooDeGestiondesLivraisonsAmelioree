...
class Picking(models.Model):
    ...
    #Ajout Boris
    file = fields.Binary(string='Inserer le bon de livraision signé  ici', attachment=True,
                         state_livraision={'validation': [('required', True)]})
    file_name = fields.Char("File Name")

    codesms1= fields.Integer(string="Code du sms recu du client",default=0)
    codesms2= fields.Integer(string="Code a envoyer au client", default=0, readonly=True)
    mobile_phone = fields.Char(string='Telephone', related='partner_id.phone',readonly=False)
                                          
    state_livraision = fields.Selection([('draftlivr', 'Brouilon'),
                                        ('code_generator', 'Code Generé'),
                                         ('send_sms', 'Sms Envoyé'),
                                         ('signature', 'Signé'),
                                         ('validation', 'Validé'),
                                        ('delivery', 'Livré')], 'Statut de la livraision',
                                        required=True, default= "draftlivr")
    ...
    def _check_file(self):
        if str(self.file_name.split(".")[1]) != 'pdf':
            raise UserError(_("Inserer uniquement un document PDF ici"))
    ...
    def code_generator(self):
        for picking in self:
            picking.codesms2 = random.randint(100000, 999999)
            self.write({'state_livraision': 'code_generator'})
        # raise UserError(_(" Votre code a été généré avec sucess."))
        # list = []
        # if picking.codesms2 not  in list:
        #     list.append(picking.codesms2)
        # else:
        #     picking.codesms2  = random.randint(10000000,99999999)

    def send_sms(self):
        api_key = 'aXVnZXQ6aXVHZXRAMjAxOQ=='
        url = 'https://app.techsoft-web-agency.com/sms/api'
        senderID = 'IUGET'
        destination = str(self.mobile_phone)
        message = "M./Mme " + self.partner_id.name + " votre facture numero" + str(self.name) + " a comme code " \
                  + str(self.codesms2) + ". Bien vouloir le communiquer au livreur pour confirmation de votre livraison"
        #message = 'M. Alain Votre commande Numero 9039738 a pour code 4567494 bien vouloir le  transmettre a votre livreur'
        sms_body = {
            'action': 'send-sms',
            'api_key': api_key,
            'to': destination,
    ...
    def code_validator(self):
        for picking in self:
            if picking.codesms2 == picking.codesms1:
                self.write({'state_livraision': 'validation'})
            else:
                raise UserError(_("Code incorrect, Veillez entrez le code de 8 caracteres recu par le client par SMS."))
